...
try:
    ...
    print("Error: customtkinter module not found. Please install it with: pip install customtkinter")
    # Fallback to standard tkinter with similar interface
    import tkinter as tk
    class FallbackCTk:
        def __getattr__(self, name):
            if name in ['CTkFrame', 'CTkLabel', 'CTkEntry', 'CTkButton', 'CTkScrollbar']:
...
try:
    ...
    print("Warning: PIL module not found. Image loading will be disabled.")
    Image = None
...
def build_header(parent):
    ...
    """
    frame = ctk.CTkFrame(parent, corner_radius=theme['radius'], fg_color=theme['header_bg'])
    frame.grid(row=0, column=0, columnspan=4, sticky='ew', padx=PADDING + 8, pady=(PADDING + 6, SPACING + 6))
        ...
        for logo_path in logo_paths:
            try:
                if logo_path.exists():                    
                    ...
                    lbl = ctk.CTkLabel(frame, image=ctk_img, text="", bg_color=theme['header_bg'])
    ...
    if not logo_found:
        lbl = ctk.CTkLabel(frame, text="🗂️", font=(FONT_FAMILY, 32), bg_color=theme['header_bg'])
        lbl.grid(row=0, column=0, padx=(PADDING + 6, SPACING + 6), rowspan=2)
    
    # Main header text
    title = ctk.CTkLabel(frame, text='Pierce County Electronic Records Classifier', font=(FONT_FAMILY, 22, 'bold'), text_color=theme['fg'])
    ...
    version = ctk.CTkLabel(frame, text=version_text, font=(FONT_FAMILY, 12), text_color=theme['fg'], anchor='w')
    version.grid(row=1, column=1, sticky='w', pady=(0, 2))
    
    return frame

def build_trust_panel(parent, show_how):
    frame = ctk.CTkFrame(parent, corner_radius=theme['radius'], fg_color=theme['panel_bg'])
    # Ensure the frame spans the full width
    frame.grid(row=1, column=0, columnspan=4, sticky='ew', padx=(PADDING + 8, PADDING + 8), pady=(SPACING, SPACING))
    frame.grid_columnconfigure(0, weight=1)
    lbl = ctk.CTkLabel(frame, text='🔒 Data never leaves your device or communicates outside of the Pierce County network. All LLM analysis is done offline and locally on your computer.', font=theme['font'], text_color=theme['accent'], wraplength=1200, anchor='w', justify='left')
    lbl.grid(row=0, column=0, padx=(PADDING, SPACING), sticky='ew')
    link = ctk.CTkLabel(frame, text='[How it works]', font=(FONT_FAMILY, 11, 'underline'), text_color=theme['fg'], cursor='hand2')
...
def build_model_controls(parent, model_var):
    ...
    try:
        model_dropdown = ctk.CTkComboBox(
            frame,
            values=["pierce-county-records-classifier-phi2", "pierce-county-records-classifier-gemma3"],
            variable=model_var,
            width=280,
...
def build_table(parent, on_sort):
    ...
    frame = ctk.CTkFrame(parent, fg_color=theme['panel_bg'], corner_radius=theme['radius'], border_width=0)
    frame.grid_propagate(False)
    
    # Column definitions
    cols = ['FileName', 'Extension', 'FullPath', 'LastModified', 'SizeKB', 'ModelDetermination', 'ConfidenceScore', 'ContextualInsights']
    ...
    for prop, value in style_props.items():
        try:
            ...
            print(f"Warning: Could not configure Treeview {prop}: {e}")
    ...
    for prop, value in heading_props.items():
        try:
            ...
            print(f"Warning: Could not configure Treeview.Heading {prop}: {e}")
    
    # Configure style maps
    try:
        style.map('Treeview.Heading', background=[('active', theme['accent_hover'])])
    except tk.TclError as e:
        print(f"Warning: Could not map Treeview.Heading background: {e}")
        
    try:
        style.map('Treeview', background=[('selected', theme['accent_hover'])])
    except tk.TclError as e:
        print(f"Warning: Could not map Treeview background: {e}")
    ...
    try:
        ...
        print(f"Warning: Could not configure alternating row styles: {e}")
    
    # Remove the borders if possible
    try:
        style.layout('Treeview', [('Treeview.treearea', {'sticky': 'nswe'})])
    except tk.TclError as e:
        print(f"Warning: Could not modify Treeview layout: {e}")
    
    # Create the treeview
    try:
        tree = ttk.Treeview(frame, columns=cols, show='headings', selectmode='browse', style='Treeview')
    except tk.TclError:
    ...
    try:
        ...
        def insert_with_tags(parent, index, **kwargs):
            try:
                ...
                print(f"Warning: Error in custom insert method: {e}")
        ...
        try:
            ...
            print(f"Warning: Could not set insert_row attribute: {e}")
    except Exception as e:
        print(f"Warning: Could not configure row tagging: {e}")
    ...
    try:
        ...
        print(f"Warning: Could not create vertical scrollbar: {e}")
    ...
    try:
        ...
        print(f"Warning: Could not create horizontal scrollbar: {e}")
        # Fallback to standard Tkinter scrollbar
    ...
    tree['displaycolumns'] = ['FileName', 'Extension', 'LastModified', 'SizeKB', 'ModelDetermination', 'ConfidenceScore']
    ...
    try:
        ...
        setattr(frame, 'default_columns', ['FileName', 'Extension', 'LastModified', 'SizeKB', 'ModelDetermination', 'ConfidenceScore'])
        setattr(frame, 'detailed_columns', cols)
    except Exception as e:
        print(f"Warning: Could not set frame attributes: {e}")
    ...
    def toggle_detail_view(show_details=None):
        ...
        try:
            default_cols = getattr(frame, 'default_columns', 
                ['FileName', 'Extension', 'LastModified', 'SizeKB', 'ModelDetermination', 'ConfidenceScore'])
            ...
            print(f"Warning: Could not toggle detail view: {e}")
            
    # Attach the method to the frame
    try:
        setattr(frame, 'toggle_detail_view', toggle_detail_view)
    except Exception as e:
        print(f"Warning: Could not set toggle_detail_view attribute: {e}")
