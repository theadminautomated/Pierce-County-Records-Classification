...
try:
    ...
    logger.info(f"Logic directory: {logic_dir}")
    logger.info(f"Logic directory exists: {os.path.exists(logic_dir)}")
    ...
    spec2 = importlib.util.spec_from_file_location(
        "file_scanner", os.path.join(logic_dir, "file_scanner.py")
    )
    ...
    logger.info("Successfully imported classification modules using importlib")
except Exception as e:
    logger.info(f"Critical import failure: {e}")
    raise
...
class SetupScreen(ctk.CTkFrame):
    ...
    def __init__(
        ...
        self.steps = (
            ...
            else [{"name": "Initializing", "weight": 1}]
        )
    ...
    def _create_ui(self):
        ...
        self.progress_rect = self.canvas.create_rectangle(
            0, 0, 0, 12, fill=theme.get("accent", "#0078d4"), width=0
        )
    ...
    def _check_ollama_service(self) -> bool:
        ...
        try:
            ...
            if result == 0:
                ...
                self.set_progress(
                    0, 50, "Ollama service not detected, attempting to start..."
                )
    ...
    def _verify_model(self) -> bool:
        ...
        try:
            ...
            success, response = _make_http_request(
                f"http://localhost:{self.ollama_port}/api/tags", method="GET"
            )
    ...
    def set_progress(self, step_index, progress_in_step, message=None):
        ...
        if 0 <= step_index < len(self.steps):
            ...
            self.current_progress = (
                prior_steps_weight + (step_weight * (progress_in_step / 100))
            ) / self.total_weight
    ...
    def _update_progress_ui(self, step_index, message=None):
        ...
        self.progress_label.configure(
            text=f"{int(self.current_progress * 100)}% complete"
        )
        self.update()
        ...
        def _do_complete():
            if success:
                ...
                self.canvas.itemconfig(
                    self.progress_rect, fill=theme.get("error", "#dc2626")
                )
    ...
    def _on_canvas_resize(self, event):
        ...
        """
        self.canvas.coords(
            self.progress_rect, 0, 0, event.width * self.current_progress, event.height
        )
...
class MainScreen(ctk.CTkFrame):
    ...
    def _setup_controls(self, parent):
        ...
        ctk.CTkLabel(
            controls_frame, text="Input Folder:", font=(FONT_FAMILY, 12, "bold")
        ...
        button_frame.grid(
            row=2, column=0, columnspan=2, sticky="ew", padx=20, pady=(10, 15)
        )
        ...
        ctk.CTkLabel(button_frame, text="Lines Per File:", font=(FONT_FAMILY, 11)).pack(
            side="left", padx=(20, 5), anchor="center"
        )

        self.lines_slider = ctk.CTkSlider(
            button_frame, from_=10, to=500, number_of_steps=490, width=120
        )
        self.lines_slider.set(100)
        self.lines_slider.pack(side="left", padx=(0, 10), anchor="center")

        self.lines_label = ctk.CTkLabel(
            button_frame, text=f"{int(self.lines_slider.get())}", font=(FONT_FAMILY, 11)
        )
        ...
        ctk.CTkLabel(
            button_frame, text="Modified \u2265 (years):", font=(FONT_FAMILY, 11)
        ).pack(side="left", padx=(20, 5), anchor="center")

        self.threshold_slider = ctk.CTkSlider(
            button_frame, from_=1, to=10, number_of_steps=9, width=120
        )
    ...
    def update_ui_sync(self, file_result, total_files):
        ...
        def _update():
            try:
                logger.debug(
                    "update_ui_sync called for file: %s",
                    file_result.get("FileName", "Unknown"),
                )
                logger.debug("File result keys: %s", list(file_result.keys()))
                logger.debug(
                    "Current tree children count: %s",
                ...
                logger.debug("Tree now has %s children", len(children))
                if children:
                    self.tree.see(children[-1])
                    logger.debug("Scrolled to last item: %s", children[-1])

                # Update progress stats
                self._sim_items_processed += 1
                self.progress_text.configure(
                    text=f"{self._sim_items_processed} files processed"
                )
                logger.debug(
                    "Updated progress text: %s files processed",
                    self._sim_items_processed,
                )
                ...
                if status == "success":
                    ...
                    logger.debug("Success count updated to: %s", self.success_count)
                elif status == "skipped":
                    self.skipped_count += 1
                    self.skipped_var.set(str(self.skipped_count))
                    logger.debug("Skipped count updated to: %s", self.skipped_count)
                else:
                    self.error_count += 1
                    self.error_var.set(str(self.error_count))
                    logger.debug("Error count updated to: %s", self.error_count)

                # Force UI refresh to show changes immediately
                self.update_idletasks()
                logger.debug("Called update_idletasks to force UI refresh")

            except Exception as e:
                logger.info(f"ERROR: Error updating UI: {e}")
                import traceback
        ...
        logger.debug(
            "Scheduling UI update for file: %s",
            file_result.get("FileName", "Unknown"),
        )
    ...
    async def _process_file(self, file_path):
        ...
        """
        try:
            logger.debug("_process_file called for: %s", file_path)
...
**Example:**  '[Quote relevant text]' and '[Quote relevant text]'"""
                ...
                logger.debug(
                    "About to call classification_engine.classify_file with model: %s",
                    self.model,
                )
                ...
                logger.debug(
                    "classification_engine.classify_file returned: %s",
                ...
                logger.debug("Final file_result: %s", file_result)
                return file_result

        except Exception as e:
            logger.info(f"ERROR: Exception in _process_file for {file_path}: {e}")
            import traceback
    ...
    def _stop_classification(self):
        ...
        logger.debug("_stop_classification called")
        self.processing = False

        if (
            hasattr(self, "_classification_task")
            and self._classification_task
            and not self._classification_task.done()
        ):
            try:
                logger.debug(
                    "Cancelling classification task: %s", self._classification_task
                )
                # Cancel the task
                ...
                def finalize_stop():
                    try:
                        logger.debug("finalize_stop called")
                        ...
                        if (
                            ...
                            if self._classification_task.cancelled():
                                logger.debug(
                                    "Classification task was successfully cancelled"
                                )
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        "Classification task completed with cancellation"
                                    )
                                except Exception as e:
                                    logger.debug(
                                        "Classification task completed with error: %s",
                                        e,
                                    )
                        ...
                        logger.info(f"ERROR: Error in finalize_stop: {e}")
                        import traceback
                        ...
                        try:
                            ...
                            logger.info(f"ERROR: Even fallback UI update failed: {e2}")

                # Give the async task time to clean up before updating UI
                logger.debug("Scheduling finalize_stop after 150ms")
                self.after(150, finalize_stop)

            except Exception as e:
                logger.info(f"ERROR: Error stopping classification: {e}")
                import traceback
                ...
                try:
                    ...
                    logger.info(f"ERROR: Immediate fallback UI update failed: {e2}")
        else:
            ...
            try:
                ...
                logger.info(f"ERROR: UI update failed: {e}")
    ...
    def export_results(self):
        ...
        self.export_btn.configure(
            text="Exported", fg_color=theme.get("button_success", "#28A745")
        )

        # Reset after delay
        def _reset_export_button():
            self.export_btn.configure(
                text=original_text, fg_color=theme.get("button_success", original_color)
            )
    ...
    def _setup_status_bar(self, parent):
        ...
        status_bar = ctk.CTkFrame(
            parent,
            fg_color=theme.get("statusbar_bg", "#0f1724"),
            height=theme.get("statusbar_height", 32),
        )
        ...
        ctk.CTkLabel(
            status_bar,
            text="Success:",
            text_color=theme.get("subtle_fg", "#7dd3fc"),
            font=(FONT_FAMILY, 11),
        ).grid(row=0, column=2, sticky="e")
        ctk.CTkLabel(
            status_bar,
            textvariable=self.success_var,
            text_color=theme.get("success", "#28A745"),
            font=(FONT_FAMILY, 11, "bold"),
        ...
        ctk.CTkLabel(
            status_bar,
            textvariable=self.skipped_var,
            text_color=theme.get("warning", "#FFC107"),
            font=(FONT_FAMILY, 11, "bold"),
        ).grid(row=0, column=5, sticky="w", padx=(2, 10))
        ...
        ctk.CTkLabel(
            ...
            text_color=theme.get("error", "#DC3545"),
            font=(FONT_FAMILY, 11, "bold"),
        ).grid(row=0, column=7, sticky="w", padx=(2, 10))
    ...
    async def classify_files(self):
        ...
        try:
            logger.debug("Starting classify_files method")
            ...
            def update_initial_status():
                ...
                logger.debug("Set initial status to 'Enumerating files...'")
            ...
            async for file in self.enumerate_files(folder, threshold_years=thresh):
                if not self.processing:  # Check if stopped
                    logger.debug("Processing stopped during enumeration")
                    return
                ...
                if len(files_batch) >= batch_size:
                    logger.debug("Processing batch of %s files", len(files_batch))
                    ...
                    def update_progress_status():
                        ...
                        logger.debug(
                            "Updated status - Found %s files", total_files_estimate
                        )
                    ...
                    for file_path in files_batch:
                        if not self.processing:
                            logger.debug("Processing stopped during batch processing")
                            return

                        try:
                            logger.debug("About to process file: %s", file_path.name)
                            # Process the file
                            file_result = await self._process_file(file_path)
                            logger.debug(
                                "File processing completed, result: %s",
                                file_result.get("Status", "Unknown"),
                            )
                            ...
                            if file_result is None:
                                logger.debug(
                                    "File processing returned None for %s",
                                    file_path.name,
                                )
                                error_result = {
                                    "FileName": (
                            ...
                            logger.debug(
                                "Calling update_ui_sync for file: %s",
                                file_path.name,
                            )
                            self.update_ui_sync(file_result, total_files_estimate)
                            processed_count += 1
                            logger.debug("Processed count now: %s", processed_count)
                            ...
                            def update_file_status():
                                ...
                                logger.debug(
                                    "Updated file status for: %s", file_path.name
                                )
                            ...
                            logger.debug(
                                "Exception processing file %s: %s", file_path, e
                            )
                            ...
                            logger.debug(
                                "Calling update_ui_sync for error result: %s",
                                file_path.name,
                            )
                            self.update_ui_sync(error_result, total_files_estimate)
                            processed_count += 1
                    ...
                    # Clear the batch
                    logger.debug(
                        "Clearing batch, processed %s files so far", processed_count
                    )
                    files_batch = []
            ...
            logger.debug(
                "File enumeration complete. Total files found: %s",
                total_files_estimate,
            )

            # Process any remaining files in the last partial batch
            if files_batch and self.processing:
                logger.debug("Processing final batch of %s files", len(files_batch))
                for file_path in files_batch:
                    if not self.processing:
                        logger.debug("Processing stopped during final batch")
                        break

                    try:
                        logger.debug("Processing final batch file: %s", file_path.name)
                        file_result = await self._process_file(file_path)
                        logger.debug(
                            "Final batch processing completed for: %s",
                            file_path.name,
                        )
                        self.update_ui_sync(file_result, total_files_estimate)
                        processed_count += 1
                        ...
                        logger.debug(
                            "Exception in final batch for %s: %s", file_path, e
                        )
                        error_result = {
                            "FileName": (
                                file_path.name
                            ...
                            "Extension": (
                                file_path.suffix if hasattr(file_path, "suffix") else ""
                            ),
                            "FullPath": str(file_path),
                            "LastModified": "Unknown",
                            "SizeKB": 0,
            ...
            logger.debug(
                "All processing complete. Total processed: %s", processed_count
            )
            if self.processing:  # Only update if not stopped
                ...
                def update_complete_status():
                    ...
                    self._update_action_buttons_visibility()
                    logger.debug(
                        "Set completion status - %s files processed",
                        processed_count,
                    )
    ...
    async def enumerate_files(self, folder, *, threshold_years: int | None = None):
        ...
        """
        try:
            logger.debug("Starting file enumeration in folder: %s", folder)
            file_count = 0
            cutoff = None
            ...
            for file in folder.rglob("*"):
                ...
                if not self.processing:
                    logger.debug(
                        "File enumeration stopped (processing=False), found %s files",
                        file_count,
                    )
                ...
                if file.is_file():
                    if (
                        cutoff
                        and datetime.fromtimestamp(file.stat().st_mtime) >= cutoff
                    ):
                        continue
                    file_count += 1
                    logger.debug(
                        "Yielding file #%s: %s (size: %s bytes)",
                        file_count,
                        file.name,
                        file.stat().st_size,
                    )
            ...
            logger.debug("File enumeration was cancelled")
            return
        except Exception as e:
            logger.info(f"ERROR: Error during file enumeration: {e}")
            import traceback
