...
def safe_theme_color(theme_dict: Dict[str, str], key: str, fallbacks: List[str] = None, default: str = "#202020") -> str:
...
from RecordsClassifierGui.gui.widgets import ToolTip,  CompletionPanel, build_run_button, build_table, build_output_selector, build_folder_selector, build_trust_panel, build_header
...
def export_results_to_csv(results: list, csv_path: str):
    ...
    fieldnames = ["File Name", "Extension", "LLM Determination", "Justification", "Confidence Score", "File Path"]
...
def process_folder_and_export(folder: str, csv_path: str, include_ext: Set[str] = INCLUDE_EXT, exclude_ext: Set[str] = EXCLUDE_EXT, max_lines: int = 100):
    """
...
class AnimatedSpinner(tk.Canvas):
    def __init__(self, parent, size=32, color='#4fa3f7', speed=0.08, **kwargs):
        ...
        self.speed = speed
        self.angle = 0
        self.arc = self.create_arc(4, 4, size-4, size-4, start=0, extent=270, style='arc', outline=color, width=4)
        self.running = False
    def start(self):
        self.running = True
        self._animate()
...
class AnimatedRunButton(ctk.CTkButton):
    ...
    def _show_checkmark(self):
        ...
        self.after(900, lambda: self.configure(text=orig_text, fg_color=self._default_color))
...
class RecordsClassifierGui(ctk.CTk):
    ...
    def __init__(self):
        ...
        style.configure('TNotebook.Tab', padding=[12, 4], font=('Segoe UI Variable', 10))
        ...
        if os.path.exists(logo_path):
            try:
                ...
                logo_label = tk.Label(self.main_app_frame, image=logo_photo, bg=theme['bg'])
                logo_label.image = logo_photo  # Keep a reference!
                logo_label.pack(pady=(20, 0))
            except Exception as e:
                print(f"Logo loading error: {e}")        # Ensure theme has all required keys before initializing UI
        ensure_theme_keys()
        ...
        try:
            ...
            if self.processing:
                ...
                if self.stats_vars['processing_times']:
                    avg_time = sum(self.stats_vars['processing_times']) / len(self.stats_vars['processing_times'])
                ...
                if hasattr(self, 'total_size'):
                    self.stats_vars['total_size'].set(f"{self.total_size / 1024:.2f} KB")
        except Exception as e:
            print(f"Error updating stats: {str(e)}")
            # Don't update stats this cycle, but continue updates
        finally:
            # Safely check for the processing attribute
            if getattr(self, 'processing', False) or getattr(self, 'always_update_stats', False):
    ...
    def _initialize_stats_panel(self, parent):
        ...
        stats_frame = ctk.CTkFrame(parent, fg_color=theme['panel_bg'], corner_radius=12, height=120)
    ...
    def _do_update_model_status_ui(self):
        ...
        if hasattr(self, 'model_status_label'):
            color = theme['success'] if self._model_status['available'] else theme['error']
        ...
        if hasattr(self, 'mode_menu'):
            if self._model_status['available']:
                self.mode_menu.configure(
                    values=["Full Classification", "DESTROY Only"],
                    state="normal"
                )
            else:
                self.mode_menu.configure(
        ...
        try:
            ...
            if self.setup_screen:
                ...
                self.setup_screen.canvas.itemconfig(
                    self.setup_screen.progress_rect,
                    fill=theme['error']
                )
            return False
    ...
    def _import_model_silently(self, model_name):
        ...
        try:
            ...
            try:
                import ollama
                models = ollama.list()
                self._model_status = {"available": False, "message": "Checking model..."}
                self._update_model_status_ui()
            except ImportError:
                self._model_status = {"available": False, "message": "Ollama package not installed"}
                self._update_model_status_ui()
                return False
            except Exception as e:
                if "connection refused" in str(e).lower():
                    self._model_status = {"available": False, "message": "Ollama service not running"}
                else:
                    self._model_status = {"available": False, "message": f"Service error: {str(e)[:50]}"}
                self._update_model_status_ui()
                return False
            ...
            self._model_status = {"available": False, "message": "Searching for Modelfile..."}
            self._update_model_status_ui()
            ...
            modelfile_path = next((path for path in possible_paths if path.exists()), None)
            if not modelfile_path:
                self._model_status = {"available": False, "message": "Modelfile not found"}
                self._update_model_status_ui()
                return False

            # Step 4: Try API import
            try:
                self._model_status = {"available": False, "message": "Creating model via API..."}
                self._update_model_status_ui()
                ...
                self._model_status = {"available": True, "message": "Model created successfully"}
                self._update_model_status_ui()
                return True
                
            except Exception as api_err:
                # Step 5: Fall back to CLI import
                self._model_status = {"available": False, "message": "Trying CLI import..."}
                self._update_model_status_ui()
                ...
                try:
                    ...
                    if process.returncode == 0:
                        self._model_status = {"available": True, "message": "Model created via CLI"}
                        self._update_model_status_ui()
                        return True
                    else:
                        error_msg = process.stderr or process.stdout or "Unknown error"
                        self._model_status = {"available": False, "message": f"CLI import failed: {error_msg[:50]}"}
                        self._update_model_status_ui()
                        return False
                        
                except Exception as cli_err:
                    self._model_status = {"available": False, "message": f"CLI attempt failed: {str(cli_err)[:50]}"}
                    self._update_model_status_ui()
                    return False
                    
        except Exception as e:
            self._model_status = {"available": False, "message": f"Import error: {str(e)[:50]}"}
            self._update_model_status_ui()
            return False
    ...
    def _add_readme_tabs(self):
        ...
        how_it_works_text = st.ScrolledText(how_it_works_frame, wrap='word', font=(FONT_FAMILY, 12), bg=theme['panel_bg'], fg=theme['fg'])
    ...
    def _initialize_ui(self):
        ...
        self.footer = ctk.CTkFrame(outer_container, fg_color=theme["statusbar_bg"], height=25)
    ...
    def _initialize_items_panel(self, parent):
        ...
        for col in columns:
            self.items_table.heading(
                col[0],
                command=lambda c=col[0]: self._sort_table(c)
            )
    ...
    def _sort_table(self, column):
        ...
        # Get all items
        items = [(self.items_table.item(child)["values"], child) 
        ...
        if column in ["size", "confidence"]:
            ...
            # Date sorting
            items.sort(key=lambda x: datetime.strptime(x[0][col_id], "%Y-%m-%d %H:%M:%S"), reverse=reverse)
        else:
            # String sorting
        ...
        for index, (values, item) in enumerate(items):
            ...
            if values[5].lower() in ["retain", "destroy", "review"]:  # determination column
