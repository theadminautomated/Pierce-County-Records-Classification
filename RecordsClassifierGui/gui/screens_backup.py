...
class SetupScreen(ctk.CTkFrame):
    ...
    """    def __init__(self, parent, on_complete=None, steps=None, task_name="Initializing", auto_run=False):
        ...
        if auto_run:
            self.after(100, self.start)    def _create_ui(self):
        ...
        self.canvas.bind('<Configure>', self._on_canvas_resize)    def _check_ollama_service(self) -> bool:
        ...
        try:
            ...
            return False    def _start_ollama_service(self) -> bool:
...
class MainScreen(ctk.CTkFrame):
    ...
    Provides the primary interface for file selection, classification, and results viewing.
    """
    def __init__(self, parent, **kwargs):
        ...
        self.model = "pierce-county-records-classifier-phi2:latest"  # Set a default model name
        self.input_folder = ""
        self.results_data = []
        ...
        self._toggle_lock = threading.Lock() # Add this lock
    ...
    def _setup_results_table(self, parent):
        ...
        # Create treeview
        columns = ('File', 'Size', 'Modified', 'Classification', 'Confidence', 'Status')
        self.tree = ttk.Treeview(table_container, columns=columns, show='headings', height=10)
        
        # Configure columns
        col_widths = {'File': 200, 'Size': 80, 'Modified': 120, 'Classification': 150, 'Confidence': 80, 'Status': 100}
        for col in columns:
    ...
    def update_ui_sync(self, file_result, total_files):
        ...
        def _update():
            try:
                # Insert file result into the table with 6 columns matching the table definition
                # Table columns: ('File', 'Size', 'Modified', 'Classification', 'Confidence', 'Status')
    ...
    async def _process_file(self, file_path):
        print(f"[DEBUG] _process_file: Entered for {file_path.name}") # DEBUG
        try:
            # Run the blocking file processing in a thread pool
            loop = asyncio.get_event_loop()
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                # Use the imported process_file function from Electronic-Records-Classification
                print(f"[DEBUG] _process_file: Calling process_file for {file_path.name} in executor") # DEBUG
                result = await loop.run_in_executor(executor, process_file, str(file_path))
                print(f"[DEBUG] _process_file: process_file returned for {file_path.name}, result: {result is not None}") # DEBUG
                ...
                if result:
                    print(f"[DEBUG] _process_file: Successfully processed {file_path.name}") # DEBUG
                    file_result.update({
                        'ModelDetermination': result.get('classification', 'Unknown'),
                        'ConfidenceScore': result.get('confidence_score', 'N/A'),
                        'ContextualInsights': result.get('reasoning', 'No insights available'),
                        'Status': 'success'
                    })
                else:
                    print(f"[DEBUG] _process_file: Processing failed for {file_path.name} (no result)") # DEBUG
                    file_result.update({
            ...
            print(f"[DEBUG] _process_file: Error processing file {file_path.name}: {e}") # DEBUG
            return {
                'FileName': file_path.name,
    ...
    def _stop_classification(self):
        ...
        print(f"[DEBUG] _stop_classification: Entered. Current self.processing: {self.processing}") # DEBUG
        self.processing = False # Set to False immediately
        print(f"[DEBUG] _stop_classification: self.processing now set to False.") # DEBUG
        
        if hasattr(self, '_classification_task') and self._classification_task and not self._classification_task.done():
            print("[DEBUG] _stop_classification: Active _classification_task found. Calling cancel().") # DEBUG
            self._classification_task.cancel()
            print("[DEBUG] _stop_classification: _classification_task.cancel() called.") # DEBUG
        else:
            if not hasattr(self, '_classification_task'):
                print("[DEBUG] _stop_classification: _classification_task attribute does not exist.") # DEBUG
            elif not self._classification_task:
                print("[DEBUG] _stop_classification: _classification_task is None.") # DEBUG
            elif self._classification_task.done():
                print("[DEBUG] _stop_classification: _classification_task is already done.") # DEBUG
        
        # Update UI
        self.run_button.configure(text="Start Classification")
        self.status_text.configure(text="Classification stopped")
        self._update_action_buttons_visibility()
        print("[DEBUG] _stop_classification: Exiting.") # DEBUG
    ...
    def _refresh_table(self):
        # If you have a refresh method, ensure it also calls:
        self._update_action_buttons_visibility()
    ...
    def _browse_folder(self):
        ...
        if folder:
            ...
            self._validate_inputs()
            # Optionally, update file stats or trigger scan here

    def _toggle_classification(self):
        """Toggle between starting and stopping classification."""
        print(f"[DEBUG] _toggle_classification: Attempting to acquire lock. Current self.processing: {self.processing}")
        if not self._toggle_lock.acquire(blocking=False):
            print("[DEBUG] _toggle_classification: Could not acquire lock, another toggle is in progress. Exiting.")
            return

        try:
            print(f"[DEBUG] _toggle_classification: Lock acquired. Current self.processing: {self.processing}")
            if self.processing:
                print("[DEBUG] _toggle_classification: self.processing is True. Calling _stop_classification()")
                self._stop_classification() 
            else:
                print("[DEBUG] _toggle_classification: self.processing is False. Attempting to start new classification.")
                # Ensure button remains enabled during processing
                self.run_button.configure(state="normal", text="Stop Classification")
                # self.processing will be set to True by the called methods

                if self.mode_var.get() == "Last Modified":
                    print("[DEBUG] _toggle_classification: Mode is 'Last Modified'. Calling _last_modified_classification()")
                    self._last_modified_classification()
                else:
                    print("[DEBUG] _toggle_classification: Mode is 'Classification'. Calling _start_classification()")
                    self._start_classification()
        finally:
            self._toggle_lock.release()
            print("[DEBUG] _toggle_classification: Lock released.")
        # Note: self.processing state here might not reflect the true state if an async op just started
        # The run_button state is now managed by the start/stop/completion logic directly.
        print(f"[DEBUG] _toggle_classification: Exiting method. Current self.run_button state: {self.run_button.cget('state')}")
    ...
    def _setup_status_bar(self, parent):
        ...
        status_bar.grid_columnconfigure(3, weight=0)
        status_bar.grid_columnconfigure(4, weight=0)
        status_bar.grid_columnconfigure(5, weight=0)
        status_bar.grid_columnconfigure(6, weight=0)
        status_bar.grid_columnconfigure(7, weight=0)
    ...
    def _setup_keyboard_shortcuts(self):
        """Bind keyboard shortcuts for accessibility and productivity using the toplevel window."""
    ...
    async def classify_files(self):
        ...
        print("[DEBUG] classify_files: Entered")
        try:
            self._sim_items_processed = 0  # Reset counter
            folder = Path(self.input_folder)
            files_to_process = []
            print("[DEBUG] classify_files: Starting file enumeration")
            async for file in self.enumerate_files(folder):
                if not self.processing:  # Check if stopped
                    print("[DEBUG] classify_files: Processing stopped during enumeration")
                    break
                files_to_process.append(file)

            print(f"[DEBUG] classify_files: Enumerated {len(files_to_process)} files")

            if files_to_process and self.processing:
                print("[DEBUG] classify_files: Starting to process chunks")
                await self.process_files_in_chunks(files_to_process)
                print("[DEBUG] classify_files: Finished processing chunks")
            elif not files_to_process:
                print("[DEBUG] classify_files: No files to process")
            elif not self.processing:
                print("[DEBUG] classify_files: Processing was stopped before chunks")

            if self.processing:  # Only update if not stopped
                print("[DEBUG] classify_files: Processing complete, updating UI")
                self.processing = False
            ...
            print("[DEBUG] classify_files: Task was cancelled.")
            self.run_button.configure(state="normal", text="Start Classification")
            self.status_text.configure(text="Classification cancelled")
            self._update_action_buttons_visibility()
        except RuntimeError as e:
            print(f"[DEBUG] classify_files: RuntimeError encountered: {e}")
            self.run_button.configure(state="normal", text="Start Classification")
            self.status_text.configure(text="Classification error")
            self._update_action_buttons_visibility()
        finally:
            print("[DEBUG] classify_files: Final cleanup")
            self.processing = False
            self.run_button.configure(state="normal", text="Start Classification")
            self.status_text.configure(text="Classification stopped")
            self._update_action_buttons_visibility()
    ...
    async def enumerate_files(self, folder):
        """Chunked file enumeration with error handling and real-time UI updates."""
        try:
    ...
    async def process_files_in_chunks(self, files_to_process, chunk_size=10):
        ...
        print(f"[DEBUG] process_files_in_chunks: Starting to process {len(files_to_process)} files in chunks of {chunk_size}")
        ...
        try:
            ...
            for i in range(0, len(files_to_process), chunk_size):
                if not self.processing:
                    print("[DEBUG] process_files_in_chunks: Processing stopped by user")
                    break
                    
                chunk = files_to_process[i:i + chunk_size]
                print(f"[DEBUG] process_files_in_chunks: Processing chunk {i//chunk_size + 1}, files {i+1}-{min(i+chunk_size, total_files)} of {total_files}")
                
                # Process each file in the current chunk
                for file_path in chunk:
                    if not self.processing:
                        print("[DEBUG] process_files_in_chunks: Processing stopped during chunk")
                        break
                        
                    try:
                        print(f"[DEBUG] process_files_in_chunks: Processing file: {file_path.name}")
                        ...
                        processed_count += 1
                          # Update status with real-time progress showing current file being processed
                        ...
                        print(f"[DEBUG] process_files_in_chunks: Error processing {file_path}: {e}")
            ...
            print(f"[DEBUG] process_files_in_chunks: Unexpected error: {e}")
            error_message = f"Error during processing: {e}"
            def update_error_status():
                self.status_text.configure(text=error_message)
            self.after(0, update_error_status)
        finally:
            print(f"[DEBUG] process_files_in_chunks: Finished processing. Processed {processed_count} of {total_files} files")
            # Update final status with proper variable capture
    ...
    def _start_classification(self):
        ...
        print("[DEBUG] _start_classification: Entered")
        try:
            ...
            print("[DEBUG] _start_classification: Input folder validated")

            if self._classification_task and not self._classification_task.done():
                print("[DEBUG] _start_classification: Cancelling existing task")
                self._classification_task.cancel()

            self.processing = True
            self.run_button.configure(text="Stop Classification")
            self.status_text.configure(text="Classification in progress...")
            self.progress_text.configure(text="0 files processed")
            self.tree.delete(*self.tree.get_children())  # Clear previous results
            self._update_action_buttons_visibility()

            loop = asyncio.get_event_loop()
            print("[DEBUG] _start_classification: Creating asyncio task for classify_files")
            self._classification_task = loop.create_task(self.classify_files())
            print("[DEBUG] _start_classification: Asyncio task created")
        except Exception as e:
            print(f"[DEBUG] _start_classification: Error in setup: {e}")
            self.processing = False            self.run_button.configure(text="Start Classification")
            self.status_text.configure(text="Error starting classification")
            self._update_action_buttons_visibility()
    
    def _validate_inputs(self, input_folder: str) -> None:
        """Validate the input folder and other required parameters."""
        from core.model_output_validation import validate_required_fields, validate_type

        # Validate input folder
        validate_type(input_folder, str)
        if not os.path.isdir(input_folder):
            raise ValueError(f"Input folder '{input_folder}' does not exist or is not a directory.")

        # Example validation for other parameters (extend as needed)
        required_fields = ["model", "input_folder"]
        validate_required_fields({"model": self.model, "input_folder": input_folder}, required_fields)

        # Additional domain-specific validations can be added here
        # For example, checking file extensions or folder permissions
    
    def _last_modified_classification(self):
        """Handle classification based on last modified date with real-time UI updates."""
        print("[DEBUG] _last_modified_classification: Entered")
        ...
        self.processing = True
        self.run_button.configure(text="Stop Classification")
        self.status_text.configure(text="Starting classification...")
        self.tree.delete(*self.tree.get_children())  # Clear previous results
        ...
        try:
            ...
            print(f"[DEBUG] _last_modified_classification: Error starting task: {e}")
            self.processing = False
            self.run_button.configure(text="Start Classification")
            self.status_text.configure(text="Error starting classification")
            
        print("[DEBUG] _last_modified_classification: Completed")
