...
def classify_with_model(content, lines_per_file=100, source_file="unknown.txt"):
    ...
    try:
        ...
        if isinstance(response, dict) and 'label' in response and 'score' in response:
            ...
            score = 0.0
            notes = raw
            # TODO: Add regex/NLP parsing if needed

        text = content
        timestamp = datetime.datetime.utcnow().isoformat() + 'Z'
        
        # Gather classification details
        keywords_found = [kw for kw in model_output_validation.SCHEDULE_6_KEYWORDS.get(label.upper(), []) 
                      if kw.lower() in text.lower()]
        model_confidence = score
        keyword_confidence = model_output_validation.compute_keyword_confidence(text, label)
        hybrid_conf = model_output_validation.hybrid_confidence(model_confidence, text, label)
